...
def measure_algorithm_time(algorithm, text, pattern):
    # Заміряємо час виконання алгоритму
    time_taken = timeit(lambda: algorithm(text, pattern), number=10) / 10
    return time_taken

# Функція для обчислення LPS (найбільшої префікс-суфіксної частини) для алгоритму КМП
